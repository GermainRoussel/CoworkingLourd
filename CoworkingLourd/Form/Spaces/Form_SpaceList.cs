...
{
    public partial class Form_SpaceList : Form
    {
        #region Déclaration des variables
        private Form_SpaceModify modifyForm;
        private bool isSortedAscending = true;

        #endregion

        #region Constructeur    
        public Form_SpaceList()
        {
        ...
        public async void LoadData(string searchTerm = "")
        {
            ...
            try
            {
                List<SpacesDto> datas = await GetDataAsync(searchTerm);
                RefreshData(datas);
            }
            catch (Exception ex)
            {
        ...
        private async Task<List<SpacesDto>> GetDataAsync()
        {
            try
            {
                var apiSpaces = new ApiSpaces();
                return await apiSpaces.GetSpacesAsync();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Erreur lors du chargement des données : " + ex.Message);
        ...
        private async Task<List<SpacesDto>> GetDataAsync(string searchTerm = "")
        {
            try
            {
                var apiSpaces = new ApiSpaces();
                return await apiSpaces.GetSpacesAsync(searchTerm);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Erreur lors du chargement des données : " + ex.Message);
        ...
        private void SpacesGridTab_Clicked(object sender, EventArgs e)
        {
        ...
        {
            if (sender is SpacesGridTab clickedTab)
            {
                ...
                if (int.TryParse(clickedTab.Tag?.ToString(), out int Id))
                {
                    ...
                    if (modifyForm == null || modifyForm.IsDisposed)
                    {
                        ...
                        modifyForm = new Form_SpaceModify(Id);
        ...
        private void RefreshData(List<SpacesDto> sortedData)
        {
            ...
            foreach (var data in sortedData)
            {
                ...
                tab.Clicked += SpacesGridTab_Clicked;
        ...
        {
            textBox_search.Text = "";
            foreach (SpacesGridTab tab in panel_todata.Controls.OfType<SpacesGridTab>())
            {
                tab.Visible = true;
            }
        }
        // Méthode pour trier les données en fonction du nom de l'entreprise
        private async void label_spacename_Click(object sender, EventArgs e)
        {
            try
            {
                // Inversez l'état du tri
